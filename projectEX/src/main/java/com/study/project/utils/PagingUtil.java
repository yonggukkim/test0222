...
public class PagingUtil {
	...
	public static Map getPageObject(int totalCount, int currentPageNo, int countPerPage, int unitPage) {
		...
		int currStartCount;
		int currEndCount;
		if (currPage != 1) { currEndCount = currPage * countPerPage;
		currStartCount = currEndCount - countPerPage; 
			...
			if (totalPage <= unitPage) {
				...
				} else { currEndPage = (currPage - 1) / unitPage * unitPage + unitPage;
			...
			boolean prePage_is;
			if (currStartPage != 1) {
				prePage_is = true; prePage = currStartPage - 1; 
				} else {
					prePage_is = false; prePage = 0; 
					}
			int nextPage;
			boolean nextPage_is;
			if (currEndPage != totalPage) {
				nextPage_is = true; nextPage = currEndPage + 1; 
				} else {
					nextPage_is = false; nextPage = 0; 
					}
			Map tempJSON = new java.util.HashMap();
			try { tempJSON.put("currPage", Integer.valueOf(currPage));
			...
			} catch (Exception localException) { } return tempJSON; } 
			/** max 페이지 구하기 **/ 
