...
public class BoardServiceImpl implements BoardService{
	...
	public String list(Map<String, Object> map, Model model) {
		...
		if(map.get("datepicker") == "" && map.get("datepicker1") == "") {
		...
		if(firstPage == 1) {
			...
			map.put("firstPage", nowPage);
		}
		
		System.out.println(map.get("firstPage").toString());
		int limit = 7;
		int limitPage = 4;
		...
		// 현재 보여줄 페이지 수
		System.out.println(((double)firstPage/(double)limitPage));
		int block = (int) Math.ceil((double)firstPage/(double)limitPage);
